...
const ProdutoInfo = ({ produtoExemplo }) => {
	...
	useEffect(() => {
		if (!produtoExemplo) {
			const fetchProduto = async () => {
				...
				try {
					...
					if (response.data?.coresProduto?.length > 0) {
						const coresPromises = response.data.coresProduto.map(
							async (cor) => {
								return cor.hexId;
							}
						);
			...
			setImagemPrincipal(
				produtoExemplo.imagensProduto[0]
					? URL.createObjectURL(produtoExemplo.imagensProduto[0].codigoImagem)
					: "https://ircsan.com/wp-content/uploads/2024/03/placeholder-image.png"
			);

			let hexIds = [];

			if (produtoExemplo.cores.length > 0) {
				for (var i = 0; i < produtoExemplo.cores.length; i++) {
					hexIds.push(produtoExemplo.cores[i].hexId);
				}
			}
	...
	return (
		<div className={styles["conteiner"]}>
			<div className={styles["button-voltar"]}>
				<h3 onClick={handleButtonClick}>Voltar</h3>
				<RiHeart3Fill
			...
			<div className={styles["conteiner-info"]}>
								...
								<>
									<div className={styles["carousel"]}>
													...
													<div
														...
														onClick={() =>
															trocarImagemPrincipal(
																produtoExemplo
																	? URL.createObjectURL(imagem.codigoImagem)
																	: imagem.codigoImagem
															)
														}
													>
										...
										<button
											...
											onClick={handleNext}
											disabled={
												currentIndex >= produto.imagensProduto.length - 4
											}
										></button>
									</div>
								</>
				...
				<div className={styles["info-produto"]}>
					<h1 className={styles["nome-produto"]}>
